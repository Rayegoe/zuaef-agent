...
from zuaef_agent.knowledge_store import MAX_SEARCH_RESULTS, KnowledgeStore
...
def test_document_first_write_has_no_semantic_gate(tmp_path: Path):
    """v1.2 T007: the kernel store has no type enum, no source-requirement
    gate, and no truth taxonomy — it is a safe document container."""
    store = KnowledgeStore(tmp_path)
    for knowledge_id in ("concepts/x", "notes/n", "sources/youtube-abc123"):
        rel = store.write_doc(
            knowledge_id=knowledge_id,
            title="t",
            body="body with real source: https://example.test/a#sec-1",
        )
        assert rel == f"knowledge/{knowledge_id}.md"
...
def test_filesystem_protects_knowledge_area():
    ...
    assert capability is not None  # sole generic write entry point for knowledge/**