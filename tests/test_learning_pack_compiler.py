...
REAL_PACK = Path(
    "/home/barry/下载/zuaef-writing-learning-pack-v0.1/zuaef-writing-learning-pack"
)
ACTIONS = [
...
def run_compiler(
    pack, sources, techniques, out, benchmark=None
...
def write_fixture_pack(
    ...
    *,
    ...
    raw_missing: AbstractSet[str] = frozenset(),
) -> Path:
    pack = root / "pack"
    ...
    sources = {sid: url for sid, (_c, _r, url) in FIXTURE_SOURCES.items()}
...
def technique_fixture(i: int) -> dict:
    ...
    if i % 3 == 0:
        ...
        mode, triggers, contexts = (
            "sensor",
            ["summary_pressure" if i % 2 else "template_connectors"],
            [],
        )
    else:
        mode, triggers, contexts = (
...
def write_curated(
    ...
    *,
    ...
    source_records = (
        ...
        if sources is not None
        else [
            {
                ...
                if sid.startswith("sanlian")
                else "research-paper",
                "url": url,
                "curated_path": FIXTURE_SOURCES[sid][0],
                "raw_path": FIXTURE_SOURCES[sid][1],
                "raw_required": True,
            }
    ...
    tech_records = (
        ...
        else [technique_fixture(i) for i in range(1, 21)]
    )
    ...
    rows = rows or [
        {
            "task_id": f"T{i:02d}",
            "sequence": i,
            "source": ["iterater", "tetra", "wpb", "re3"][i % 4],
        }
        for i in range(1, 21)
    ]
...
class TestHappyPaths:
    def test_happy_path_with_benchmark(self, fixture, tmp_path):
        _root, pack, sp, tp, bench = fixture
    ...
    def test_pack_only(self, fixture, tmp_path):
        _root, pack, sp, tp, _bench = fixture
        out = tmp_path / "compiled"
    ...
    def test_optional_raw_missing_normalized_to_null(self, tmp_path):
        ...
        sp, tp = write_curated(
            ...
            mutate_sources=lambda recs: [
                {**r, "raw_required": False} if r["id"] == "sanlian-a" else r
                for r in recs
            ],
        )
        out = tmp_path / "compiled"
        ...
        sources = [
            json.loads(line)
            for line in (out / "sources.jsonl").read_text(encoding="utf-8").splitlines()
        ]
        a = next(s for s in sources if s["id"] == "sanlian-a")
        assert (
            a["raw_path"] is None
            and a["raw_sha256"] is None
            and a["raw_locator"] is None
        )
...
class TestValidationFailures:
    def test_missing_curated_source_file(self, fixture, tmp_path):
        _root, pack, _sp, _tp, _bench = fixture
        sp2, tp2 = write_curated(
            tmp_path / "v1",
            pack,
            mutate_sources=lambda recs: [
                {**r, "curated_path": "sanlian/absent.md"}
                if r["id"] == "sanlian-a"
                else r
                for r in recs
            ],
        ...
        _assert_fail(
            run_compiler(pack, sp2, tp2, tmp_path / "out"), "curated file missing"
        )
    ...
    def test_required_raw_missing(self, tmp_path):
        ...
        sp, tp = write_curated(tmp_path, pack)
        _assert_fail(
            run_compiler(pack, sp, tp, tmp_path / "out"), "required raw file missing"
        )

    def test_malformed_curated_sources_jsonl(self, fixture, tmp_path):
        _root, pack, _sp, tp, _bench = fixture
        bad = tmp_path / "bad_sources.jsonl"
        bad.write_text('{"id": "x"\n', encoding="utf-8")
        _assert_fail(
            run_compiler(pack, bad, tp, tmp_path / "out"), ":1: malformed JSON"
        )

    def test_malformed_curated_techniques_jsonl(self, fixture, tmp_path):
        _root, pack, sp, _tp, _bench = fixture
        bad = tmp_path / "bad_tech.jsonl"
        bad.write_text("not-json\n", encoding="utf-8")
        _assert_fail(
            run_compiler(pack, sp, bad, tmp_path / "out"), ":1: malformed JSON"
        )
    ...
    def test_duplicate_source_id(self, fixture, tmp_path):
        ...
        _assert_fail(
            run_compiler(pack, sp2, tp2, tmp_path / "out"), "duplicate source id"
        )
    ...
    def test_duplicate_technique_id(self, fixture, tmp_path):
        ...
        _assert_fail(
            run_compiler(pack, sp2, tp2, tmp_path / "out"), "duplicate technique id"
        )
    ...
    def test_missing_t020(self, fixture, tmp_path):
        ...
        _assert_fail(
            run_compiler(pack, sp2, tp2, tmp_path / "out"), "missing technique id"
        )
    ...
    def test_invalid_action(self, fixture, tmp_path):
        ...
        sp2, tp2 = write_curated(
            ...
            mutate_techniques=lambda recs: [
                {**r, "action": "rewrite_everything"} if r["id"] == "T001" else r
                for r in recs
            ],
        )
        _assert_fail(
            run_compiler(pack, sp2, tp2, tmp_path / "out"), "not in frozen five"
        )
    ...
    def test_invalid_trigger_signal(self, fixture, tmp_path):
        ...
        _assert_fail(
            run_compiler(pack, sp2, tp2, tmp_path / "out"), "not in frozen five sensors"
        )
    ...
    def test_invalid_activation_combination(self, fixture, tmp_path):
        ...
        sp2, tp2 = write_curated(
            ...
            mutate_techniques=lambda recs: [
                ...
                for r in recs  # T003 fixture is context mode
            ],
        )
        _assert_fail(
            run_compiler(pack, sp2, tp2, tmp_path / "out"),
            "mode=context requires empty",
        )
    ...
    def test_unknown_technique_source_id(self, fixture, tmp_path):
        ...
        sp2, tp2 = write_curated(
            ...
            pack,
            mutate_techniques=lambda recs: [
                {**r, "sources": ["ghost-source"]} if r["id"] == "T001" else r
                for r in recs
            ],
        )
    ...
    def test_primary_source_not_in_sources(self, fixture, tmp_path):
        ...
        sp2, tp2 = write_curated(
            ...
            pack,
            mutate_techniques=lambda recs: [
                {**r, "primary_source": "sanlian-b"} if r["id"] == "T005" else r
                for r in recs
            ],
        )
...
class TestLegacyMismatch:
    def test_legacy_source_url_mismatch(self, fixture, tmp_path):
        ...
        sp, tp = write_curated(tmp_path, pack)
        _assert_fail(
            run_compiler(pack, sp, tp, tmp_path / "out"), "legacy url mismatch"
        )
    ...
    def test_legacy_name_mismatch(self, fixture, tmp_path):
        ...
        sp, tp = write_curated(tmp_path, pack)
        _assert_fail(
            run_compiler(pack, sp, tp, tmp_path / "out"), "legacy name mismatch"
        )
    ...
    def test_legacy_source_mismatch(self, fixture, tmp_path):
        ...
        sp, tp = write_curated(tmp_path, pack)
        _assert_fail(
            run_compiler(pack, sp, tp, tmp_path / "out"), "legacy source mismatch"
        )
...
class TestBenchmarkJoin:
    def test_bad_source_join(self, fixture, tmp_path):
        _root, pack, sp, tp, bench = fixture
        rows = [
            json.loads(line) for line in bench.read_text(encoding="utf-8").splitlines()
        ]
        rows[0]["source"] = "nonexistent-source"
        bad = write_benchmark(tmp_path / "b1", rows=rows)
        _assert_fail(
            run_compiler(pack, sp, tp, tmp_path / "out", benchmark=bad),
            "no exact match",
        )
    ...
    def test_source_with_zero_candidates(self, fixture, tmp_path):
        pack = write_fixture_pack(
            tmp_path, extra_sources={"empty-src": "https://example.com/empty"}
        )
        _sp, tp = write_curated(tmp_path, pack)
                ...
                *recs,
                {
                    ...
                    "curated_path": "sanlian/empty-src.md",
                    "raw_path": "raw/empty-src.md",
                    "raw_required": True,
                },
            ],
        )
            ...
            rows=[
                {
                    ...
                    "source": "empty-src"
                    if i == 1
                    else ["iterater", "tetra", "wpb", "re3"][i % 4],
                }
                for i in range(1, 21)
            ],
        )
        _assert_fail(
            run_compiler(pack, sp2, tp, tmp_path / "out", benchmark=bench),
...
class TestDeterminismAndHygiene:
    ...
    def test_evidence_loads_through_real_store(self, fixture, tmp_path):
        ...
        assert all(
            e.weight == 0.75 and e.approved_by == "pack-curation:v0.1" for e in corpus
        )
    ...
    def test_no_absolute_path_in_compiled(self, fixture, tmp_path):
        ...
        for f in out.iterdir():
            assert "/home/" not in f.read_text(encoding="utf-8") and str(
                tmp_path
    ...
    def test_no_raw_body_field_in_compiled(self, fixture, tmp_path):
        ...
        for name in (
            ...
            for line in (out / name).read_text(encoding="utf-8").splitlines():
                rec = json.loads(line)
                assert not (
                    {"body", "full_text", "raw_text", "raw_body", "text"} & set(rec)
                ), name
...
class TestRealCompiledSnapshot:
    def test_snapshot_counts_and_abi(self):
        ...
        sources = [
            json.loads(line)
            for line in (compiled / "sources.jsonl")
            .read_text(encoding="utf-8")
            .splitlines()
        ]
        techs = [
            json.loads(line)
            for line in (compiled / "techniques.jsonl")
            .read_text(encoding="utf-8")
            .splitlines()
        ]
        evidence = [
            json.loads(line)
            for line in (compiled / "evidence.jsonl")
            .read_text(encoding="utf-8")
            .splitlines()
        ]
        sequential = [
            json.loads(line)
            for line in (compiled / "sequential_inputs.jsonl")
            .read_text(encoding="utf-8")
            .splitlines()
        ]
        assert (len(sources), len(techs), len(evidence), len(sequential)) == (
            14,
            20,
            20,
            20,
        ...
        assert all(
            s["raw_sha256"] for s in sources
        )  # v0.1: all raw required and present
