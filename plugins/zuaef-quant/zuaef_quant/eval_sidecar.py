"""Deterministic strategy evaluation sidecar (P1/P5.9-D).

Production authority lives here.  This module is executed only in the
isolated ``.venv-quant`` side environment; the Agent main environment never
imports it and never carries qlib/pandas.
...
Runs in the .venv-quant side environment (qlib + akshare):
    ...
    PYTHONPATH=plugins/zuaef-quant .venv-quant/bin/python \
        -m zuaef_quant.eval_sidecar \
