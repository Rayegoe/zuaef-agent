...
platform). `--state-dir` isolates fixture/replay runs from real results.
    ...
    .venv/bin/python -m zuaef_quant.monitor once
    .venv/bin/python -m zuaef_quant.monitor session --interval 45
    .venv/bin/python -m zuaef_quant.monitor ack-buy --symbol 600000 --price 10.5 --shares 500 --venue paper
    .venv/bin/python -m zuaef_quant.monitor ack-sell --symbol 600000 --price 10.9 --shares 500
    .venv/bin/python -m zuaef_quant.monitor skip --symbol 600000 --price 10.5 --note "too extended"
    .venv/bin/python -m zuaef_quant.monitor status
"""
...
from zuaef_quant.freshness import derive_freshness
from zuaef_quant.quant_core import StrategySpec, ensure_history, load_config, read_cache
from zuaef_quant.scan import entry_clause_gaps as _shared_entry_clause_gaps
from zuaef_quant.scan import evaluate_entry
...
def clause_distances(pullback: float, ratio: float, strength: float, price: float, spec: StrategySpec) -> dict:
    """Compatibility wrapper over the one shared clause-gap authority.

    The formula lives in ``zuaef_quant.scan.entry_clause_gaps``; this adapter
    only maps the monitor's frozen ``StrategySpec`` onto that engine. Every
    production caller (monitor NEAR band, symbol context) delegates here so
    no second copy of the arithmetic can drift.
    """
    return _shared_entry_clause_gaps(
        pullback,
        ratio,
        strength,
        price,
    ...
    def close_position(self, position: dict, price: float, shares: int, when: str) -> dict:
        ...
        closed = {
            **position,
            ...
            "exit_shares": shares,
            "exit_time": when,
            "pnl": position_pnl(position, price, shares=shares),
        }
...
def run_cycle(
    ...
    *,
    ...
    for symbol in symbols:
        ...
        trigger = bool(evaluated["trigger"])
        distances = evaluated["clause_distances"]
        near = bool(evaluated["near"])
    ...
    for position in list(store.positions["open"]):
        ...
        positions_live[symbol] = mark_to_market(position, price)
...
def cmd_symbol_context(args, store: Store) -> int:
    ...
    if quote and float(quote.get("price", 0) or 0) > 0:
        ...
        try:
            quote_date = datetime.strptime(quote_day, "%Y%m%d").replace(tzinfo=TZ_SH).date()
        except ValueError:
            quote_date = None
    ...
    hist, _hist_meta = read_cache("daily", f"{symbol}_qfq")
    hist_close = (
        pd.to_numeric((hist.sort_values("date") if "date" in hist else hist)["close"], errors="coerce").dropna()
    ...
    # "历史不足"
    history_block = {
        "bars_available": len(hist_close) if hist_close is not None else 0,
        "required_bars": 25,
        "hydration_status": hydration["status"],
        "sufficient": bool(
            (len(hist_close) if hist_close is not None else 0) >= 25
        ),
    }
