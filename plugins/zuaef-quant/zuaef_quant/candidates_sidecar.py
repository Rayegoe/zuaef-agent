"""Deterministic candidate discovery sidecar (P5.9-C).
...
live trigger (``zuaef_quant.scan_sidecar``).

    PYTHONPATH=plugins/zuaef-quant .venv-quant/bin/python \
        -m zuaef_quant.candidates_sidecar [--refresh]
...
from .scan import entry_trigger
from .scan_sidecar import (
    ACTIVE_SYMBOLS_PATH,
...
def score_timing(
    ...
    if entry_trigger(
        ...
        ratio,
        strength,
        entry_pullback_max=entry_max,
        entry_volume_ratio_min=entry_vol,
    ):
        state = "TRIGGER"
