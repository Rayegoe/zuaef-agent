#!/usr/bin/env python3
"""ZUAEF-ASHARE-001 · dashboard transport (loopback-only, P5.9-F).

Production authority lives here.  This server owns only HTTP transport and
access to rendered/static business artifacts; business strategy, candidate,
position, validation and scan logic stay in their domain authorities.


GET / , /business , /index.html → docs/quant/business.html   (业务决策页, 默认)
GET /engineering               → docs/quant/dashboard.html   (工程/审计页)
GET /api/scan                  → 活跃候选宇宙实时扫描 JSON (确定性; zuaef_quant.scan_sidecar)
...
写接口是 canonical trading host 操作 (zuaef_quant.monitor) 的薄
HTTP adapter，不做任何 JSON 直写；且强制 loopback-only —— --host 非本机
回环地址时 POST 一律 403 (无账号系统，Phase 1 不对 LAN 暴露写接口)。

用法:
    PYTHONPATH=plugins/zuaef-quant python3 -m zuaef_quant.dashboard.serve [--port 8787]
...
from ..runtime import package_parent, resolve_quant_python, resolve_repo_root
from . import render as biz
...
def _quant_python() -> Path:
    """The isolated side environment that owns scan/monitor execution."""
    return resolve_quant_python()


#: Public command shapes kept as module constants for existing operator/test
#: compatibility.  Production work is always a `-m zuaef_quant.*` module
#: invoked by the quant Python, never a root tools script.
SCAN_CMD = [str(resolve_quant_python()), "-m", "zuaef_quant.scan_sidecar"]
WATCHLIST_CMD = SCAN_CMD + ["--universe-file", LEGACY_WATCHLIST]
MONITOR_CMD = [str(resolve_quant_python()), "-m", "zuaef_quant.monitor"]


def _side_env() -> dict[str, str]:
    """Make the plugin package importable inside the side environment."""
    env = os.environ.copy()
    plugin_parent = str(package_parent())
    previous = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = plugin_parent + (os.pathsep + previous if previous else "")
    return env


def scan_command() -> list[str]:
    return list(SCAN_CMD)


def watchlist_command() -> list[str]:
    return list(WATCHLIST_CMD)


def monitor_command() -> list[str]:
    return list(MONITOR_CMD)
...
def ack_command_for(kind: str, payload: dict) -> list[str]:
    ...
    argv = monitor_command() + ["--state-dir", TRADING_STATE_DIR, kind, "--symbol", symbol]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            r = subprocess.run(
                cmd, cwd=str(REPO_ROOT), env=_side_env(),
