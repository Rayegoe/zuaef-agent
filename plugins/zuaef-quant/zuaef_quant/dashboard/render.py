...
(``zuaef_quant.candidates_sidecar``), last live-scan snapshot, Decision Briefs,
...
Stdlib only: runs with plain python3, no quant dependency group needed.
    ...
    PYTHONPATH=plugins/zuaef-quant python3 -m zuaef_quant.dashboard.render \
        [--out docs/quant/business.html]
...
# Shared domain projection authority (stdlib-only module): the dashboard must
# not independently parse forward.json counts.  The package root is resolved
# without depending on the repository's tool-checkout topology.
...
def load_real_trend(
    ...
    *,
    ...
    m1 = {
        ...
        "formal_forward_count": forward_counts["count"],
        "formal_forward_settled": forward_counts["settled_all_kinds"],
    }
    oks = [market_ok, single_ok, bool(in_session)]
    m1["verdict"] = (
        "PASS" if all(oks) and forward_counts["count"] > 0
        else "PARTIAL" if any(oks) or forward_counts["count"]
    ...
    return {
        ...
        "universe_size": universe_size,
        "forward": {
            "present": forward_counts["present"],
            "count": forward_counts["count"],
            "settled": forward_counts["settled_all_kinds"],
        },
        "m1_evidence": m1,
...
def _pit_dimension(pit_audit: dict | None) -> dict:
    ...
    if pit_audit is None:
        return {"status": "UNKNOWN", "detail": "尚未运行 PIT 审计 (developer proof tooling)"}
...
def _semantic_dimension(proof: dict | None, snapshot: dict | None) -> dict:
    ...
    """
    if proof is None:
        return {"status": "UNKNOWN", "detail": "尚未运行语义验证 (developer proof tooling)"}
...
def data_quality(
    ...
    if not snapshot:
        return {
            ...
            "degraded": True,
            "dimensions": {},
            "banner": "候选快照缺失 — 请先运行: .venv-quant/bin/python -m zuaef_quant.candidates_sidecar",
        }
...
# shared by the static page and GET /api/quant/now via zuaef_quant.dashboard.serve)
# --------------------------------------------------------------------------
...
def _latest_v31_experiment(v31_dir: Path) -> Path | None:
    research_dir = Path(v31_dir) / "research"
    if not research_dir.is_dir():
        return None
    dirs = sorted(
        (p for p in research_dir.iterdir() if p.is_dir() and (p / "decision.json").exists()),
        key=lambda p: p.name,
    )
...
GLOSSARY = {
    ...
    "composite_score": {
        ...
        "s": "zuaef_quant.candidates_sidecar 确定性计算",
    },
    "tier": {
        "t": "分级",
    ...
    "candidates": {
        ...
        "s": "zuaef_quant.candidates_sidecar",
    },
    "eligible": {
        "t": "合格 / 硬排除",
...
<div class="wrap">
  <header>
    ...
    <div class="sub">市场机会 · 候选证据 · 实时时序 · 前向结果 · 生成于 <span id="gen-time"></span> · 再生成: <code>python3 -m zuaef_quant.dashboard.render</code></div>
  </header>
...
function renderNow(n){
  ...
  if(!n.present){
    grid.innerHTML = '<div class="n"><div class="k">Runtime</div><div class="v" style="color:var(--amber)">UNKNOWN</div></div>' +
      '<div class="n"><div class="k">说明</div><div class="v" style="font-size:12px">启动: zuaef-quant monitor once</div></div>';
...
function renderAttention(n){
  ...
  const state = n || {};
  if(!state.present){
    statusEl.textContent = '盯盘监控未运行 — 启动: zuaef-quant monitor once';
...
async function submitTrade(){
  ...
  try{
    ...
    msg.style.color = 'var(--red)';
    msg.textContent = '无法连接 zuaef-quant dashboard serve — 记录交易需运行: zuaef-quant dashboard serve（静态文件模式不产生任何写入）';
  }
}
...
document.addEventListener('click', (e)=>{
  ...
  if(act === 'ask'){
    ...
    const promptText = `分析${sym}：先调用 get_symbol_context 读取该标的的当前事实；如果问题涉及已有持仓或退警，再调用 get_positions。解释当前 deterministic 状态（触发条件、风险、失效条件），区分 Runtime/Agent/Human 三态；不要宣称盈利能力，不要把 SYSTEM_UNAVAILABLE 说成 NO_TRADE。`;
...
function renderRealTrend(){
  ...
  $('rt-rows').innerHTML = pts.length ? pts.map(p=>`<tr>
    ...
    : '<tr><td colspan="6" class="mut">无真实记录 — 运行 zuaef-quant monitor once 开始积累</td></tr>';
}
...
function renderStatic(){
  ...
  $('scan-meta').textContent = D.scan_meta.present
    ...
    : '无扫描快照 — 运行 zuaef-quant scan 或启动 zuaef-quant dashboard serve';
  ...
  const drawBoard = (rows) => {
    ...
    </tr>`).join('') : '<tr><td colspan="17" class="mut">候选池为空 — 这是数据/覆盖失败状态, 不是"没有机会"的市场结论。运行 .venv-quant/bin/python -m zuaef_quant.candidates_sidecar</td></tr>';
  };
  ...
  $('board-meta').textContent = D.snapshot_meta.as_of
    ...
    : '候选快照缺失 — 先运行 zuaef_quant.candidates_sidecar';
  ...
  $('formal-forward').innerHTML = f.count
    ...
    : '<tr><td colspan="6" class="mut">暂无前向观察 — 运行 zuaef-quant scan 开始积累</td></tr>';
