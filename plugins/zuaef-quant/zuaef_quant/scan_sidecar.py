"""Domain-owned deterministic live-scan sidecar (P5 operator engine).
...
Universe resolution (business-dashboard spec 03/T005) — priority:
  ...
  2. data/quant-cache/candidates/active_symbols.json if valid and non-empty
     (deterministic candidate-pool handoff from zuaef_quant.candidates_sidecar);
...
so the decision brief can state why no trigger evidence exists.
    ...
    .venv-quant/bin/python -m zuaef_quant.scan_sidecar [--max-triggers 10]
...
# The sidecar lives in the domain package; direct module execution needs the
# package parent on sys.path. The shared quant_core now lives in this package
# as well, so production scan logic no longer imports root tools.
...
from zuaef_quant.scan import entry_trigger
...
def resolve_universe(
    ...
    if active_path.exists():
        try:
            ...
            raise UniverseError(
                f"{exc}; remove the file or rerun python -m zuaef_quant.candidates_sidecar"
            ) from exc
        return {
            "symbols": symbols,
    ...
    raise UniverseError(
        ...
        "Run python -m zuaef_quant.candidates_sidecar first."
    )
