"""Domain-owned shared quant primitives (P5.8 authority extraction).
...
zuaef_quant.eval_sidecar and runs in the .venv-quant side environment.
"""
    ...
    *,
    ...
    def _attach(df: pd.DataFrame | None, meta: dict | None) -> None:
        if df is None or not isinstance(meta, dict):
            return
        evidence["bars_available"] = len(df)
        evidence["sufficient"] = len(df) >= required_bars
