"""``zuaef-knowledge-worker`` plugin factory.

Bundles the released Harness You.com capabilities (YouSearch survey +
page reads; optional YouResearch multi-step research) with a small
deterministic document toolset and plugin-local skill guidance. The
profile is the sole open-web provider for the general-knowledge-worker
deployment: the profile must keep core ``web_search``/``web_fetch`` off
so the upstream web tool names never collide.

Config is non-secret composition identity (it freezes into the
CompositionSnapshot); unknown keys fail composition instead of being
silently ignored, and ``YDC_API_KEY`` stays in the host environment.
"""
...
# The full config contract (spec pack 03 §Config contract). A key outside
# this set is a misconfiguration (or an attempt to smuggle behavior in) —
# fail composition before any model work.
...
def build_plugin(env: PluginEnv, config: dict[str, Any]) -> PluginBundle:
    ...
    if unknown:
        raise CompositionError(
            f"knowledge-worker config has unknown key(s) {unknown}; allowed: "
    ...
    raw_language = config.get("output_language", "")
    if not isinstance(raw_language, str):
        raise CompositionError("knowledge-worker config 'output_language' must be a string")
    output_language = raw_language.strip() or None
