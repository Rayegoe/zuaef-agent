...
def build_plugin(env: PluginEnv, config: dict[str, Any]) -> PluginBundle:
    ...
    guidance = FunctionToolset(instructions=(
        ...
        "Own the requested engineering outcome and act directly. "
        "Before using any repository or shell tool, decide whether the user's "
        "requested outcome actually requires inspecting, editing, testing or "
        "operating code/repository/developer tooling. "
        "If it does not, make ZERO tool calls. "
        "Do not reinterpret a business, market, writing, general-knowledge or "
        "personal question as a request to inspect the repository. "
        "For an out-of-scope request, return a short profile-mismatch response "
        "and stop immediately. "
        "A business question under the coding profile is NOT a repository "
        "investigation; equivalently: business question under coding is not "
        "repository investigation. Example: for '分析A股大跌原因', reply concisely that "
        "this is the coding profile and the request needs the corresponding "
        "market-analysis profile; do not call search_files, repo_search, "
        "repo_read, repo_run_command or search_knowledge. "
        "Repository "
