...
5. Your final message is the natural result for the current user — the host
   settles artifact byte facts and completed tool effects automatically; you
   never craft receipt or evidence fields.
"""
...
def build_writing_agent(
    ...
    *,
    ...
    usage limits, step effects, artifact byte facts, and receipt settlement.
    """
...
def main() -> int:
    ...
    verified = [(v.path, v.sha256) for v in receipt.artifact_facts]
    ...
    machine_checks = [
        (
            ...
            f"execution_state={receipt.execution_state}",
        ),
        (
        ...
        (
            ...
            ReceiptStore(settings.state_root).path_for(run_id).is_file(),
            str(ReceiptStore(settings.state_root).path_for(run_id)),
        ),
        (
            "usage recorded",
            bool(receipt.usage),
    ...
    test_complete = receipt.execution_state == "completed" and checks_ok

    print("\n=== Observed agent trajectory (from ZUAEF tool-effect ledger) ===")
    for effect in receipt.tool_effect_facts:
    ...
    print(f"  receipt    {ReceiptStore(settings.state_root).path_for(run_id)}")
