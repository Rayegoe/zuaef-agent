"""Contract tests for ACE Writing plugin v0.2."""
...
from zuaef_ace_writing import EditorialControlCapability, create_plugin
...
class TestPluginContract:
    def test_bundle_is_toolset_plus_editorial_capability(self, tmp_path: Path) -> None:
    ...
    def test_editorial_control_can_be_disabled(self, tmp_path: Path) -> None:
    ...
    def test_expected_tool_names_are_unchanged(self, tmp_path: Path) -> None:
        bundle = create_plugin(_env(tmp_path), {"ace_root": str(_fake_ace_root(tmp_path))})
    ...
    def test_parity_with_proof_toolset_is_preserved(self, tmp_path: Path) -> None:
